...
for (var i = 0; i < radar_data.length; i++) {
    ...
    var itemsByStage = _.groupBy(radar_data[i].items, function(item) {return Math.floor(item.pc.r / arc_width)});
    var offsetIndex = 0;

    for (var stageIdx in _(itemsByStage).keys()) {

        if (stageIdx > 0) {
            // if(itemsByStage[stageIdx-1] === undefined) {
            //   debugger;
            // }

            if(itemsByStage[stageIdx-1]) {
        ...
        if(radar_arcs[stageIdx] === undefined) {
          debugger;
        }
